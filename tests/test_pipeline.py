def test_variable_inputs():
    ...
    node1 = ProcessNode(
        name='node1',
        executable='node1',
        out_paths={'output': 'path1'},
        node_dpath='.',
    )
    node2 = ProcessNode(
        name='node2',
        executable='node2',
        out_paths={'output': 'path2'},
        node_dpath='.',
    )
    node3 = ProcessNode(
        name='node3',
        executable='node3',
        out_paths={'output': 'path3'},
        node_dpath='.',
    )

    combine_node = ProcessNode(
        name='combine', executable='combine', in_paths={'varpaths'}
    )

    node1.outputs['output'].connect(combine_node.inputs['varpaths'])
    node2.outputs['output'].connect(combine_node.inputs['varpaths'])
    node3.outputs['output'].connect(combine_node.inputs['varpaths'])

    dag_nodes = [node1, node2, node3, combine_node]
    ...
    print('---')
    dag.print_commands(with_status=0, with_guards=0, with_locks=0)
...
def test_slurm_options(tmp_path):
    ...
    node1 = TimedNode(
        name='node1',
        executable='node1.exe',
        out_paths={'key1': 'path1'},
        node_dpath='.',
    )
    node2 = ProcessNode(
        name='node2',
        executable='node2.exe',
        out_paths={'key2': 'path2'},
        node_dpath='.',
    )
    node3 = ProcessNode(
        name='node3',
        executable='node3.exe',
        out_paths={'key3': 'path3'},
        node_dpath='.',
    )
