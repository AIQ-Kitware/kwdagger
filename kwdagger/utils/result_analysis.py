...
class Result(ub.NiceRepr):
    ...
    def __init__(
        self, name: Any, params: Any, metrics: Any, meta: Any = None
    ) -> None:
        self.name = name
        self.params = params
    ...
    def __nice__(self) -> str:
        row = self.to_dict()
        text = cast(str, ub.urepr(row, compact=True, precision=2, sort=False))
        return text
...
class ResultTable:
    ...
    def __init__(self, params: Any, metrics: Any) -> None:
        self.params = params
        self.metrics = metrics
    ...
    def result_list(self) -> list[Result]:
        if 'result_list' not in self._cache:
            new_results = [
                ...
                for idx, (metrics, params) in enumerate(
                    zip(
                        self.metrics.to_dict('records'),
    ...
    def coerce(
        ...
        if isinstance(data, cls):
            ...
            if param_cols is None or metric_cols is None:
                raise Exception(
                    'Both param_cols and metric_cols must be given when input is a single data frame'
                )
    ...
    def varied(self) -> dict[str, dict[Any, int]]:
        ...
        varied = dict(
            varied_value_counts(
                self.params, default=sentinel, min_variations=2, dropna=True
            )
        )
        # remove nans
        # varied = {
...
class ResultAnalysis(ub.NiceRepr):
    ...
    def __init__(
        ...
        metrics: Any = None,
        params: Any = None,
        ignore_params: Any = None,
        ignore_metrics: Any = None,
        metric_objectives: Any = None,
        abalation_orders: Any = {1},
        default_objective: str = 'max',
        ...
        self.statistics: list[Any] | None = None
    ...
    def __nice__(self) -> str:
        return cast(str, ub.urepr(self._description, si=True, sv=True))
    ...
    def abaltion_groups(self, param_group: Any, k: int = 2) -> list[Any]:
        ...
        groups = []
        for key, group in fix_groupby(
            table.groupby(other_params, dropna=False)
        ):
            if len(group) >= k:
                groups.append(group)
        return groups

    def _objective_is_ascending(self, metric_key: Any) -> bool:
        """
        Args:
        ...
        if objective is None:
            warnings.warn(
                f'warning assume {self.default_objective} for {metric_key=}'
            )
    ...
    def ablate(
        self, param_group: Any, metrics: Any = None, use_openskill: Any = 'auto'
    ) -> list[Any]:
        """
        TODO:
        ...
        if self.table is None:
            raise NotImplementedError(
                'was build table not ported from geowatch?'
            )
        ...
        if metrics is None:
            ...
            metrics_of_interest = sorted(
                avail_metrics - set(self.ignore_metrics)
            )
        else:
        ...
        for group in groups:
            for metric_key in metrics_of_interest:
                ...
                scored_ranking = best_group[
                    param_group + [metric_key]
                ].reset_index(drop=True)
                scored_obs.append(scored_ranking)
                if skillboard is not None:
                    ranking = [
                        gd(**d)
                        for d in scored_ranking[param_group].to_dict('records')
                    ]
        ...
        for key, improves in score_improvements.items():
            ...
            improves_arr = np.array(improves)
            pos_delta = improves_arr[improves_arr > 0]
            print(
    ...
    def test_group(self, param_group: Any, metric_key: Any) -> dict[str, Any]:
        """
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings(
                'ignore', 'divide by zero', category=RuntimeWarning
            )
            warnings.filterwarnings(
                'ignore', 'invalid value', category=RuntimeWarning
            )
            ...
            for pair in value_pairs:
                ...
                with warnings.catch_warnings():
                    warnings.filterwarnings(
                        'ignore', 'Degrees of freedom', category=RuntimeWarning
                    )
                    warnings.filterwarnings(
                        'ignore', 'invalid value', category=RuntimeWarning
                    )
                ...
                if nuisance_cols:
                    ...
                    nk_to_group1 = dict(
                        list(fix_groupby(nuisance_vals1.groupby(nuisance_cols)))
                    )
                    nk_to_group2 = dict(
                        list(fix_groupby(nuisance_vals2.groupby(nuisance_cols)))
                    )
                else:
                ...
                if common:
                    ...
                    comparable_groups1 = metric_group1.loc[
                        comparable_indexes1, metric_key
                    ]
                    comparable_groups2 = metric_group2.loc[
                        comparable_indexes2, metric_key
                    ]
    ...
    def build(self) -> None:
        ...
        held_constant_orders_set = {
            num_varied_params + i if i < 0 else i for i in self.abalation_orders
        }
        held_constant_orders = [i for i in held_constant_orders_set if i > 0]
    ...
    def report(self) -> None:
        ...
        stat_groups = ub.group_items(
            self.statistics, key=lambda x: x['param_name']
        )
    ...
    def _report_one(self, grid_item: Any) -> None:
        ...
        stats_row = ub.group_items(stat_group, key=lambda x: x['metric'])[
            metric_key
        ][0]
        ...
        print('')
        print(
            f'ANOVA: If p is low, the param {param_name_show} might have an effect'
        )
        print(
            ub.color_text(
            ...
            if 'ttest_ind' in pairstat:
                ...
                print(
                    ub.color_text(
                        ...
                        'green'
                        if ttest_ind_result.pvalue < p_threshold
                        else None,
                    )
                )
            if 'ttest_rel' in pairstat:
                ...
                print(
                    ub.color_text(
                        ...
                        'green'
                        if ttest_rel_result.pvalue < p_threshold
                        else None,
                    )
                )
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        palette = ub.dzip(
            unique_hues, sns.color_palette(n_colors=len(unique_hues))
        )
        ...
        for fnum, (fig_key, group) in enumerate(hack_groups, start=base_fnum):
            ...
            facet_data_groups = dict(
                list(fix_groupby(facet.data.groupby(facet._col_var)))
            )
...
class SkillTracker:
    ...
    def _resolve_openskill_api(self) -> dict[str, Any]:
        import openskill

        openskill_any: Any = openskill
        if hasattr(openskill_any, 'Rating'):
            ...
            return {
                'new_rating': openskill_any.Rating,
                'rate': openskill_any.rate,
                'predict_win': openskill_any.predict_win,
            }
        else:
    ...
    def observe(self, ranking: Any) -> None:
        """
...
def varied_values(
    ...
    varied: dict[str, set[Any]] = ub.ddict(set)
    for row in longform:
        for key in columns:
...
def varied_value_counts(
    ...
    varied_counts: dict[str, Counter[Any]] = ub.ddict(Counter)
    for row in longform:
        for key in columns:
            value = row.get(key, default)
            if isinstance(value, list):
...
if 1:
    ...
    def fix_groupby(groups: Any) -> Any:
        keys = groups.keys
        if isinstance(keys, list) and len(keys) == 1:
...
def stats_dict(data: Any, suffix: str = '') -> dict[str, Any]:
    stats = {
...
def combine_stats(s1: Any, s2: Any) -> Any:
    """
...
def combine_stats_arrs(data: Any) -> dict[str, Any]:
    sizes = data['nobs']
    means = data['mean']
    stds = data['std']
    mins = data['min']
