...
def build():
    ...
    segment = ProcessNode(
        name='segment', executable='python segment.py',
        in_paths={'dataset_fpath'}, out_paths={'masks_fpath': 'masks.json'},
        algo_params={'model': 'unet'}, perf_params={'workers': 4})
    score_det = ProcessNode(
        name='score_det', executable='python score_det.py',
        in_paths={'truth_fpath', 'dets_fpath'},
        out_paths={'score_det_fpath': 'score_det.json'},
        algo_params={'iou_thresh': 0.5})
    score_seg = ProcessNode(
        name='score_seg', executable='python score_seg.py',
        in_paths={'truth_fpath', 'masks_fpath'},
        out_paths={'score_seg_fpath': 'score_seg.json'},
        algo_params={'iou_thresh': 0.5})
    summarize = ProcessNode(
        name='summarize', executable='python summarize.py',
    ...
    dag = Pipeline({'detect': detect, 'segment': segment,
