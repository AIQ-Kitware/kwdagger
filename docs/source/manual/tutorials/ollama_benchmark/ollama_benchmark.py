    ...
    def main(cls, argv=True, **kwargs):
        ...
        if ok_trials:
            ...
            lats = [
                t.latency_total_sec
                for t in ok_trials
                if t.latency_total_sec is not None
            ]
            tps_list = [
                t.tokens_per_sec
                for t in ok_trials
                if t.tokens_per_sec is not None
            ]
            ...
            eval_token_counts = [
                t.response_tokens
                for t in ok_trials
                if t.response_tokens is not None
            ]

            total_durs = [
                t.total_duration_ns
                for t in ok_trials
                if t.total_duration_ns is not None
            ]
            load_durs = [
                t.load_duration_ns
                for t in ok_trials
                if t.load_duration_ns is not None
            ]
            prompt_eval_durs = [
                t.prompt_eval_duration_ns
                for t in ok_trials
                if t.prompt_eval_duration_ns is not None
            ]
            eval_durs = [
                t.eval_duration_ns
                for t in ok_trials
                if t.eval_duration_ns is not None
            ]
            ctx_lens = [
                t.context_len for t in ok_trials if t.context_len is not None
            ]
        else:
            ttfts = lats = tps_list = []
            prompt_lens = prompt_token_counts = eval_token_counts = []
            total_durs = load_durs = prompt_eval_durs = eval_durs = (
                ctx_lens
            ) = []
        ...
        if config.append_jsonl is not None:
            ...
            print(
                f'Appended {len(trials)} rows to JSONL: {config.append_jsonl}'
            )
...
def _wait_for_ollama(
    url: str, timeout: float = 60.0, interval: float = 2.0
) -> bool:
    base = url.rstrip('/')
